    ...
    if args.is_training:
        for ii in range(args.itr):
            ...
            for i in range(1):
                ...
                # Save metrics, preds, and trues to CSV files
                np.savetxt(f'metrics_run_{i}_{args.model}_{args.pred_len}_{args.target}.csv', metrics, delimiter=',')
                np.savetxt(f'preds_run_{i}_{args.model}_{args.pred_len}_{args.target}.csv', preds, delimiter=',')
                np.savetxt(f'trues_run_{i}_{args.model}_{args.pred_len}_{args.target}.csv', trues, delimiter=',')
                
                # Append metrics, preds, and trues to the lists
            ...
            with open('mean_std_metrics_{args.model}_{args.pred_len}_{args.target}.csv', 'w', newline='') as csvfile:
                writer = csv.writer(csvfile)
                writer.writerow(['Metric', 'Mean', 'Standard Deviation'])
                for i, metric_name in enumerate(['mae', 'mse', 'rmse', 'mape', 'mspe']):
                    writer.writerow([metric_name, mean_metrics[i], std_metrics[i]])
